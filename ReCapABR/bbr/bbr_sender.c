...
static void bbr_on_network_invalidation(bbr_sender_t* s)
{
	...
	uint32_t pacing_rate_kbps, target_rate_bps, pading_rate_kbps, instant_rate_kbps, bbr_bwe_bps;
	int acked_bitrate;
	...
	bbr_bwe_bps = s->info.target_rate.bandwidth * 8000;
	bbr_bwe_bps = SU_MAX(bbr_bwe_bps, s->min_bitrate);
	bbr_bwe_bps = SU_MIN(s->max_bitrate, bbr_bwe_bps);
	s->target_bitrate=bbr_bwe_bps;
	suint32_t new_bitrate = bbr_bwe_bps;
	/*s->target_bitrate = SU_MAX(s->target_bitrate, s->min_bitrate);
	s->target_bitrate = SU_MIN(s->max_bitrate, s->target_bitrate);
	uint32_t new_bitrate = s->target_bitrate;*/
	...
	/*������ݷ����仯�����д���һ��ͨ�Ų�֪ͨ*/
	if (new_bitrate != s->last_bitrate_bps || loss != s->last_fraction_loss){
		s->last_bitrate_bps = new_bitrate;
		s->last_rtt = (uint32_t)s->info.target_rate.rtt;
		s->last_fraction_loss = loss;

		if (s->trigger != NULL && s->trigger_cb != NULL)
			//s->trigger_cb(s->trigger, bbr_bwe_bps, loss, (uint32_t)s->info.target_rate.rtt);
			s->trigger_cb(s->trigger, new_bitrate, loss, (uint32_t)s->info.target_rate.rtt);
	}
}
...
void bbr_sender_heartbeat(bbr_sender_t* s, int64_t now_ts)
{
	...
	if (s->bbr != NULL && s->notify_ts + k_bbr_heartbeat_timer < now_ts){
		s->info = bbr_on_heartbeat(s->bbr, now_ts);//putchar('H');
...
void bbr_sender_on_feedback(bbr_sender_t* s, uint8_t* feedback, int feedback_size)
{
	...
	if (s->bbr != NULL){
		...
		if (s->notify_ts + k_bbr_heartbeat_timer < now_ts){
			bbr_on_network_invalidation(s);//putchar('F');
			s->notify_ts = now_ts;
		}
	}
}
