...
static size_t bbr_get_target_congestion_window(bbr_controller_t* bbr, double gain)
{
	...
	int32_t MnRTT, EstBW;
	MnRTT = (int32_t)(bbr_get_min_rtt(bbr));
	EstBW = (int32_t)(bbr_bandwidth_estimate(bbr));
	
	bdp = (int32_t)(MnRTT * EstBW);
	congestion_window = (size_t)(gain * bdp);
	// printf("A %d :  %d %d %.6lf\n", (int)(congestion_window), (int)MnRTT, (int)EstBW, gain);
...
static int bbr_update_bandwidth_and_min_rtt(bbr_controller_t* bbr, int64_t now_ts, bbr_packet_info_t packets[], int size, uint32_t bandwidth)
{
	...
	// if (sample.is_app_limited) printf("OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOHH!!\n");
